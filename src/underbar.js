(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if (isSorted) {
      ...
      for (let i = 0; i < array.length; i++) {
        let duplicate = result.includes(array[i]);
        if (duplicate === false) {
          result.push(array[i]);
        }
      }
  ...
  _.some = function(collection, iterator) {
    ...
    if(_.every(iterated, function(value) {
      return !value;
    })) {
      return false;
    } else {
  ...
  _.shuffle = function(array) {
    ...
    let randomized = [];
    let randomInt = function(length) {
      return Math.floor(Math.random() * length);
    }
    while(copy.length) {
      let index = randomInt(copy.length - 1);
      randomized.push(copy[index]);
      copy.splice(index, 1);
    }
    return randomized;
  };
  ...
  _.invoke = function(collection, functionOrKey, args) {
    for (let i = 0; i < collection.length; i++){
      if (typeof functionOrKey === 'function') {
        ...
        collection[i] = collection[i][functionOrKey](args);
      }
    }
    return collection;
  };
